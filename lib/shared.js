...
exports.gatherZones = async (accountId, page, results) => {
  const per_page = 50;
  const all_zones = results || [];
  const page_number = page || 1;
  try {
    const response = await axios.get(`/zones?per_page=${per_page}&page=${page_number}&account.id=${accountId}`);
    ...
    if ('result_info' in response.data) {
      ...
      if (page_number <= total_pages) {
        await this.gatherZones(accountId, page_number + 1, all_zones);
      }
    }
  } catch (error) {
    axiosError(error);
...
exports.getZoneDnsRecords = async (zoneId, page, results) => {
  const per_page = 50;
  const page_results = results || [];
  const page_number = page || 1;
  try {
    const response = await axios.get(`/zones/${zoneId}/dns_records?per_page=${per_page}&page=${page_number}`);
    response.data.result.forEach((item) => {
      page_results.push(item);
    });
    // are there more pages of results?
    if ('result_info' in response.data) {
      const { total_pages } = response.data.result_info;
      if (page_number <= total_pages) {
        await this.gatherZones(zoneId, page_number + 1, page_results);
      }
    }
  } catch (error) {
    axiosError(error);
...
exports.getZoneSettings = async (zone_id) => {
  try {
    const response = await axios.get(`/zones/${zone_id}/settings`);
...
exports.getLocalZoneSettings = async (configDir) => {
  // settings are in `.settings.yaml` in `redirects` folder
...
// NOTE don't need this - as data will be retrieved via zone list
// // assuming no pagination required
// exports.getZone = async (zone_id) => {
//   try {
//     const response = await axios.get(`/zones/${zone_id}`);
//     // console.log(`zone:\n${JSON.stringify(response.data, null, '  ')}`);
//     return response.data.result;
//     // response.data.result.forEach((zone) => {
//     //   console.log('zone', zone);
